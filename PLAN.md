...
## 新闻自动发布可靠性（2026-07-13）

- 已修正：4 个日常时段各发布 1 篇，避免首个时段一次耗尽当日额度。
- 已新增：每日末段补发任务，会补齐当天尚未完成的数量，最多补发至 4 篇总配额。
- 已新增：PostgreSQL 租约锁，避免重叠的 Vercel Cron 或人工运行重复采集、重复发布。
- 已优化：新闻源按受控并发采集，图片优先采用 RSS 引用图，候选发布窗口与图片请求并发受限，降低函数超时风险。
- 保留：美洲限定、72 小时来源窗口、7 天 URL 去重、语义相似度阈值、来源/主题平衡、信息增益评分、引用与日志字段。
