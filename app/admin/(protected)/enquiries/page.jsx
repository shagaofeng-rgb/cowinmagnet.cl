...
const t = {
  ...
  country: "国家", product: "产品", status: "状态", delivery: "邮件通知", date: "提交时间", newLead: "新询盘", empty: "暂无询盘。"
};
...
export default async function AdminEnquiriesPage({ searchParams }) {
  ...
  return <section className="admin-panel">
    ...
    <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>{t.id}</th><th>{t.name}</th><th>{t.company}</th><th>{t.country}</th><th>{t.product}</th><th>{t.status}</th><th>{t.delivery}</th><th>{t.date}</th><th>访问归属</th></tr></thead><tbody>
      {enquiries.items.map((item) => <tr key={item.id}>
        <td>{item.id}</td><td>{item.name || "-"}</td><td>{item.company || "-"}</td><td>{item.country || "-"}</td>
        <td>{item.product || item.productRequirement || "-"}</td><td>{item.status || t.newLead}</td><td>{item.notification?.status === "delivered" ? "已发送" : item.notification?.status === "failed" ? "待重试" : item.notification?.status === "not_configured" ? "未配置" : "处理中"}</td><td>{item.createdAt || item.submittedAt || "-"}</td>
      ...
      {!enquiries.items.length ? <tr><td colSpan="9">{t.empty}</td></tr> : null}
    </tbody></table></div>
