...
export async function getStoredAdminAccount(email = getConfiguredAdminEmail()) {
  ...
  if (db) {
    await ensureSchema();
    const result = await db.query("SELECT email, name, password_hash FROM admin_accounts WHERE email = $1", [normalizedEmail]);
...
export async function upsertStoredAdminPassword({ email = getConfiguredAdminEmail(), name = process.env.ADMIN_NAME || "Administrator", password }) {
  ...
  if (db) {
    await ensureSchema();
    await db.query(
      `INSERT INTO admin_accounts (email, name, password_hash, created_at, updated_at)
