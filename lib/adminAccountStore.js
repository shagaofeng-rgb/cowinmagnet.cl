...
function getPool() {
  ...
  if (!pool) {
    ...
    pool.on("error", (error) => {
      console.warn("[adminAccountStore] idle database connection error", error?.message || error);
    });
  }
  return pool;
}
...
export async function getStoredAdminAccount(email = getConfiguredAdminEmail()) {
  ...
  if (db) {
    const result = await withDatabaseRetry(async () => {
      await ensureSchema();
      return db.query(`SELECT email, name, password_hash FROM ${ADMIN_TABLE} WHERE email = $1`, [normalizedEmail]);
    }, { label: "adminAccountStore.read" });
...
export async function upsertStoredAdminPassword({ email = getConfiguredAdminEmail(), name = process.env.ADMIN_NAME || "Administrator", password }) {
  ...
  if (db) {
    ...
    }, { label: "adminAccountStore.upsert" });
