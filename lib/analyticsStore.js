...
async function ensureSchema() {
  ...
  try {
    ...
    `);
    await client.query("CREATE INDEX IF NOT EXISTS analytics_events_type_date_idx ON analytics_events (type, created_at DESC)");
    schemaReady = true;
  } finally {
...
export async function appendAnalyticsEvent(payload) {
  ...
  if (db) {
    await ensureSchema();
    await db.query(
      `INSERT INTO analytics_events (id, payload, type, visitor_id, page, country, created_at)
...
async function readAnalyticsEvents() {
  ...
  if (db) {
    await ensureSchema();
    const result = await db.query("SELECT payload FROM analytics_events ORDER BY created_at DESC LIMIT 5000");
