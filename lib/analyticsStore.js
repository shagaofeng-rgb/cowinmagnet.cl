...
function visitorRows(events, allEvents, filters) {
  ...
  const rows = [...grouped.values()].map((item, index) => {
    ...
    // Do not expose the aggregation Sets to the client. Apart from not being a
    // useful transport shape, spreading `item` after `pages` used to overwrite
    // the array with a Set, which broke the visitor table tooltip at render time.
    const { sessions, pages, countries, ...visitor } = item;
    return {
      ...visitor,
...
// Health checks must not materialize every event simply to report four totals.
// This small aggregate stays bounded as analytics history grows and is safe to
// run alongside the customer-facing admin pages.
export async function getAnalyticsHealthSummary(range) {
  const db = getPool();
  if (db) {
    const result = await withDatabaseRetry(async () => {
      await ensureSchema();
      return db.query(
        `SELECT
          ...
          COUNT(DISTINCT visitor_id) FILTER (WHERE type = 'page_view')::int AS unique_visitors,
          COUNT(DISTINCT session_id) FILTER (WHERE type = 'page_view')::int AS sessions,
          COUNT(*) FILTER (WHERE type = 'form_submit')::int AS inquiries
         FROM ${ANALYTICS_TABLE}
         WHERE is_excluded = FALSE AND created_at >= $1 AND created_at <= $2`,
        [range.startDate, range.endDate]
      );
    }, { label: "analyticsStore.health-summary" });
