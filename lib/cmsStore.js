  ...
  if (db) {
    try {
      ...
      const result = await db.query(
        "SELECT payload FROM cms_items WHERE type = $1 ORDER BY published_at DESC, created_at DESC",
        [type]
      );
...
export async function updateCmsItemStatus(type, slug, status) {
  ...
  if (db) {
    await ensureSchema();
    await db.query(
      "UPDATE cms_items SET payload = jsonb_set(payload, '{status}', to_jsonb($3::text), true), updated_at = NOW() WHERE type = $1 AND slug = $2",
      [type, slug, status]
    );
    return;
  }
