...
export async function withCmsLease(lockKey, task, { leaseMs = 120000 } = {}) {
  ...
  await withDatabaseRetry(() => ensureSchema(), { label: "cmsStore.lease-schema" });
  const owner = `${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
  const acquired = await withDatabaseRetry(() => db.query(
    `
  ...
  ), { label: "cmsStore.lease-acquire" });
  if (!acquired.rowCount) return { acquired: false, value: null };

  try {
    return { acquired: true, value: await task() };
  } finally {
    await withDatabaseRetry(() => db.query(`DELETE FROM ${AUTOMATION_LOCK_TABLE} WHERE lock_key = $1 AND owner = $2`, [lockKey, owner]), { label: "cmsStore.lease-release" }).catch(() => {});
  }
}
