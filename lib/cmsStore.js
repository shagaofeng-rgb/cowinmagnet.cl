...
async function ensureSchema() {
  ...
  try {
    ...
    `);
    await client.query(`CREATE UNIQUE INDEX IF NOT EXISTS ${CMS_TABLE}_type_slug_idx ON ${CMS_TABLE} (type, slug)`);
    await client.query(`CREATE INDEX IF NOT EXISTS ${CMS_TABLE}_type_date_idx ON ${CMS_TABLE} (type, published_at DESC)`);
    schemaReady = true;
  } finally {
