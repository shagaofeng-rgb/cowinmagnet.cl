...
export async function getPendingEnquiryNotifications({ limit = 10 } = {}) {
  ...
  const db = getPool();
  if (db) {
    // Claim records before sending. Without this transaction, overlapping cron
    // invocations can both read a pending enquiry and send duplicate emails.
    return withDatabaseRetry(async () => {
      ...
      try {
        ...
        const result = await client.query(
          `SELECT id, payload FROM ${ENQUIRIES_TABLE}
           WHERE COALESCE((payload->'notification'->>'attempts')::int, 0) < 5
             AND (
               COALESCE(payload->'notification'->>'status', 'pending') IN ('pending', 'failed')
               OR (
                 payload->'notification'->>'status' = 'processing'
                 AND COALESCE(NULLIF(payload->'notification'->>'processingAt', '')::timestamptz, 'epoch'::timestamptz) < $2
               )
             )
           ORDER BY created_at ASC
           LIMIT $1
           FOR UPDATE SKIP LOCKED`,
          [safeLimit, staleBefore]
        );
        const claimed = [];
        for (const row of result.rows) {
          ...
          await client.query(
            `UPDATE ${ENQUIRIES_TABLE}
             SET payload = jsonb_set(jsonb_set(payload, '{notification}', $2::jsonb, true), '{updatedAt}', to_jsonb($3::text), true), updated_at = $3
             WHERE id = $1`,
            [row.id, JSON.stringify(notification), now]
          );
          claimed.push({ ...row.payload, notification });
        }
  ...
  const now = new Date().toISOString();
  const items = await readFileItems();
  const claimed = items.filter((item) => ["pending", "failed"].includes(item.notification?.status || "pending") && Number(item.notification?.attempts || 0) < 5).slice(0, safeLimit);
  if (claimed.length) {
    const ids = new Set(claimed.map((item) => item.id));
    await writeFileItems(items.map((item) => ids.has(item.id) ? { ...item, notification: { ...(item.notification || {}), status: "processing", processingAt: now, updatedAt: now }, updatedAt: now } : item));
  }
  return claimed.map((item) => ({ ...item, notification: { ...(item.notification || {}), status: "processing", processingAt: now, updatedAt: now } }));
}
