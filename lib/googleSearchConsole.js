...
import { getSitemapStatusRequest, submitSitemapRequest } from "./searchConsoleSitemap.mjs";
...
export async function getGoogleSearchConsoleSitemapStatus(sitemapUrl = process.env.GOOGLE_SEARCH_CONSOLE_SITEMAP_URL || "https://cowinmagnet.cl/sitemap.xml") {
  const tokenResult = await getAccessToken();
  if (!tokenResult.ok) return { success: false, status: 0, error: tokenResult.error, sitemapUrl, siteUrl: getSiteUrl() };
  return getSitemapStatusRequest({
    accessToken: tokenResult.token,
    siteUrl: getSiteUrl(),
    sitemapUrl
  });
...
async function querySearchAnalytics(token, range, dimensions = [], rowLimit = 10) {
  ...
  const response = await googleFetch(`${SEARCH_ANALYTICS_URL}/${siteUrl}/searchAnalytics/query`, {
    method: "POST",
    headers: {
...
async function inspectUrl(token, inspectionUrl) {
  let response;
  try {
    response = await googleFetch(URL_INSPECTION_URL, {
      method: "POST",
    headers: {
      authorization: `Bearer ${token}`,
      "content-type": "application/json"
    ...
    return {
      url: inspectionUrl,
      verdict: "UNKNOWN",
      coverageState: error?.name === "AbortError" ? "Inspection request timed out." : "Inspection request failed.",
      lastCrawlTime: null
    };
  }
  const body = await response.json().catch(() => ({}));
...
export async function inspectGoogleSearchConsoleUrls(urls, { concurrency = 2 } = {}) {
  const normalizedUrls = [...new Set((Array.isArray(urls) ? urls : [])
    .filter((url) => typeof url === "string" && /^https:\/\//i.test(url)))]
    .slice(0, 50);
  let configured = false;
  try {
    configured = Boolean(getServiceAccount() && getSiteUrl());
  } catch {
    configured = false;
  }
  ...
  const tokenResult = await getAccessToken();
  if (!tokenResult.ok) {
    return { configured, siteUrl: getSiteUrl(), error: tokenResult.error, inspections: [], summary: { total: normalizedUrls.length, pass: 0, excluded: 0, fail: 0, unknown: normalizedUrls.length } };
  }

  const inspections = [];
  const batchSize = Math.max(1, Math.min(Number(concurrency) || 2, 4));
  for (let index = 0; index < normalizedUrls.length; index += batchSize) {
    inspections.push(...await Promise.all(normalizedUrls.slice(index, index + batchSize).map((url) => inspectUrl(tokenResult.token, url))));
  }

  const summary = inspections.reduce((counts, item) => {
    if (item.verdict === "PASS") counts.pass += 1;
    else if (item.verdict === "EXCLUDED") counts.excluded += 1;
    else if (item.verdict === "FAIL") counts.fail += 1;
    else counts.unknown += 1;
    return counts;
  }, { total: inspections.length, pass: 0, excluded: 0, fail: 0, unknown: 0 });

  return { configured: true, siteUrl: getSiteUrl(), inspectedAt: new Date().toISOString(), inspections, summary };
}
...
export async function getGoogleSearchConsoleSnapshot(range) {
  ...
  if (!tokenResult.ok) {
    return {
      ...
      sitemap: { success: false, status: 0, error: tokenResult.error, sitemapUrl: process.env.GOOGLE_SEARCH_CONSOLE_SITEMAP_URL || "https://cowinmagnet.cl/sitemap.xml", siteUrl: getSiteUrl() },
      queries: [],
      pages: [],
      countries: [],
      devices: [],
  ...
  try {
    const [overviewRows, queryRows, pageRows, countryRows, deviceRows, sitemap] = await Promise.all([
      ...
      getSitemapStatusRequest({ accessToken: tokenResult.token, siteUrl: getSiteUrl(), sitemapUrl: process.env.GOOGLE_SEARCH_CONSOLE_SITEMAP_URL || "https://cowinmagnet.cl/sitemap.xml" })
    ]);
    ...
    return {
      ...
      overview: {
        ...
        inspectedUrlCount: indexingStatus.length
      },
      sitemap,
    ...
    return {
      ...
      error: error.message,
      overview: { clicks: 0, impressions: 0, ctr: 0, position: 0, indexedPages: 0, notIndexedPages: 0 },
      sitemap: { success: false, status: 0, error: error.message, sitemapUrl: process.env.GOOGLE_SEARCH_CONSOLE_SITEMAP_URL || "https://cowinmagnet.cl/sitemap.xml", siteUrl: getSiteUrl() },
      queries: [],
      pages: [],
      countries: [],
      devices: [],
