...
function americasRelevanceScore(candidate) {
  const text = `${candidate.title} ${candidate.summary} ${candidate.link || candidate.url || ""}`.toLowerCase();
  const places = [
...
async function fetchFeed(source) {
  ...
  return itemBlocks.map((block) => {
    ...
    const sourcePublishedAt = sourcePublishedRaw ? new Date(sourcePublishedRaw).toISOString() : "";
    return {
...
function buildNewsPost(candidate, image) {
  ...
  const productImage = candidate.relatedProducts?.[0]?.image || "";
  const coverImage = productImage || image || "/assets/home-hero-cowinmagnet-ai.jpg";
  const citations = [
  ...
  return {
    ...
    body,
    image: coverImage,
    imagePolicy: productImage ? "cowinmagnet_product_image" : "remote_hotlink_with_credit",
    sourceImageUrl: image,
    localImagePath: "",
    imageCredit: productImage ? "Cowinmagnet product catalog" : image ? candidate.sourceName || candidate.rootDomain : "",
