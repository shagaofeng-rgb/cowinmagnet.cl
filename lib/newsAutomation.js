...
async function fetchFeed(source) {
  ...
  const itemBlocks = [...xml.matchAll(/<item[\s\S]*?<\/item>/gi)].map((m) => m[0]).slice(0, 14);
...
async function chooseNewsImage(candidate) {
  ...
  const fromArticle = fromFeed || await getOgImage(candidate.link);
...
export async function runNewsAutomation({ limit = DAILY_NEWS_QUOTA, dryRun = false } = {}) {
  ...
  for (const candidate of eligible) {
    ...
    const image = await chooseNewsImage(candidate);
    if (!image.url) {
