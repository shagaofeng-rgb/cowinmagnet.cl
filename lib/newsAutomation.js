...
function americasRelevanceScore(candidate) {
  const text = `${candidate.title} ${candidate.summary} ${candidate.sourceName} ${candidate.sourceFeed}`.toLowerCase();
  const places = [
  ...
  return places.filter((key) => text.includes(key)).length + companies.filter((key) => text.includes(key)).length;
}
