...
import { activeSourcesForNewsSite } from "@/lib/newsSourceCatalog.mjs";
import { canonicalDomain } from "@/lib/newsSourceCatalog.mjs";
import { canUseSourceForCandidate } from "@/lib/newsSourceValidation.mjs";
import { parseNewsFeed as parseExternalNewsFeed } from "@/lib/newsFeedParser.mjs";
import { maxNewsDuplicationScore } from "@/lib/newsSimilarity.mjs";
...
function parseRssFeed(xml, source) {
  return [...xml.matchAll(/<item(?:\s[^>]*)?>([\s\S]*?)<\/item>/gi)].slice(0, 15).map((match) => {
    ...
    return {
      ...
      domain: canonicalDomain(url || source.rssOrApiUrl)
    };
...
function parseAtomFeed(xml, source) {
  return [...xml.matchAll(/<entry(?:\s[^>]*)?>([\s\S]*?)<\/entry>/gi)].slice(0, 15).map((match) => {
    ...
    return {
      title: clean(tag(raw, "title")),
      url,
      excerpt: clean(tag(raw, "summary") || tag(raw, "content")).slice(0, 1800),
      publishedAt: Number.isNaN(published.getTime()) ? "" : published.toISOString(),
      accessedAt: new Date().toISOString(),
      sourceName: source.name,
      sourceGroup: source.group,
  ...
  try {
    ...
    return (feed.items || []).slice(0, 15).map((item) => {
      ...
      return {
        ...
        url,
        excerpt: clean(item.summary || item.content_text || item.content_html).slice(0, 1800),
        publishedAt: Number.isNaN(published.getTime()) ? "" : published.toISOString(),
        accessedAt: new Date().toISOString(),
        sourceName: source.name,
        sourceGroup: source.group,
...
async function fetchFeed(source, site) {
  ...
  return parseExternalNewsFeed(await response.text(), source);
}
...
function sourceUsage(recent = []) {
  ...
  for (const article of recent) {
    if (new Date(article.publishedAt || article.createdAt || 0).getTime() < cutoff) continue;
    const sources = Array.isArray(article.sources) ? article.sources : [{ url: article.sourceUrl }];
    for (const source of sources) {
      const domain = canonicalDomain(source?.url || "");
...
function consecutiveUses(recent = [], field, value) {
  let count = 0;
  for (const article of [...recent].sort((a, b) => new Date(b.publishedAt || b.createdAt || 0) - new Date(a.publishedAt || a.createdAt || 0))) {
    const source = article.sources?.[0] || {};
    const actual = field === "region" ? (source.country || source.region || "").toLowerCase() : (source.sourceGroup || "").toLowerCase();
    if (actual !== String(value || "").toLowerCase()) break;
    count += 1;
  }
  return count;
}
...
export async function discoverNewsCandidates({ siteId, recentArticles = [], fallback = false } = {}) {
  ...
  const configuredSources = await activeSourcesForNewsSite(site, { fallback });
  ...
  for (const item of items) {
    ...
    const eventFingerprint = theme
      ? createHash("sha256").update(`${theme.themeId}|${tokens(item.title).sort().join("-")}`).digest("hex")
      : "";
    const duplicateScore = maxNewsDuplicationScore({
      title: item.title,
      summary: item.excerpt,
      eventFingerprint,
      sources: [{ title: item.title, supportedFact: item.excerpt }]
    }, recentArticles);
    let reason = "";
    if (ageHours < 0 || ageHours > maxAgeHours) reason = "outside_candidate_age_window";
      ...
      const sourceGate = canUseSourceForCandidate({
        active: true,
        validationStatus: "verified",
        robotsAllowed: true,
        tier: item.tier || "B"
      }, {
        domainUsesLast14Days: usage.get(item.domain) || 0,
        consecutiveGroupUses: consecutiveUses(recentArticles, "group", item.sourceGroup),
        consecutiveRegionUses: consecutiveUses(recentArticles, "region", item.country)
      });
      if (!sourceGate.eligible) reason = sourceGate.reason;
    }
    if (reason) {
