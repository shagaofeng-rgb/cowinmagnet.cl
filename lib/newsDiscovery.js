...
export async function discoverNewsCandidates({ siteId, recentArticles = [], fallback = false } = {}) {
  ...
  for (const item of items) {
    ...
    if (reason) {
      rejectedSources.push({ url: item.url, domain: item.domain, reason, duplicationScore });
      continue;
    }

    const score = informationGain(item, theme, recentArticles, usage);
    if (score < site.news.minScore) {
      rejectedSources.push({ url: item.url, domain: item.domain, reason: "information_gain_below_threshold", informationGainScore: score, duplicationScore });
      continue;
    }
