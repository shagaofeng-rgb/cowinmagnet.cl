...
export async function runEditorialPublication({ dryRun = false, force = false, generateOnly = false } = {}) {
  ...
  await saveCmsItem({ ...candidate, id: `news-${candidate.slug}`, type: "news", status: "published", idempotencyKey, publishedAt, sourceUrl: candidate.sources?.[0]?.url || "", sourceTitle: candidate.sources?.[0]?.title || "", sourcePublishedAt: candidate.sources?.[0]?.publishedAt || "", sourceFetchedAt: candidate.sources?.[0]?.accessedAt || "" });
