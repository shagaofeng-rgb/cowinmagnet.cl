...
export async function runEditorialPublication({ dryRun = false } = {}) {
  ...
  if (latest && Date.now() - new Date(latest.publishedAt || latest.createdAt).getTime() < TWO_DAYS) return { success: true, status: "waiting_48_hours", published: 0 };
  const recent = published.filter((item) => Date.now() - new Date(item.publishedAt || item.createdAt || 0).getTime() < 180 * 86400000);
  const candidate = candidates.find((item) => item.status === "quality_review" && item.editorialApproved === true);
  if (!candidate) return { success: true, status: "no_approved_candidate", published: 0 };
  ...
  if (!quality.eligible || dryRun) return { success: quality.eligible, status: run.status, published: 0, failures: quality.failures };
  ...
  await saveCmsItem({ ...candidate, type: "news", status: "published", idempotencyKey, publishedAt: new Date().toISOString() });
