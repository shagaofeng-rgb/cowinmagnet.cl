...
export function evaluateEditorialCandidate(candidate, recentArticles = []) {
  ...
  // The article body contains a stable technical selection guide. Reject only when
  // the overall candidate is genuinely near-duplicate, not when that guidance repeats.
  if (duplicationScore > 0.85) failures.push("semantic_duplicate_recent_publication");
...
async function publishOne({ dryRun = false, force = false, generateOnly = false, context = {} } = {}) {
  ...
  let candidate = state.candidates.find((item) => ["quality_review", "evidence_review", "awaiting_editorial_approval"].includes(item.status));
  let discovery = null;
  if (!candidate) {
  ...
  const run = {
    ...
    status: !quality.eligible ? "rejected" : (dryRun ? "dry_run" : "passed"),
  ...
  if (!quality.eligible || dryRun || generateOnly) {
    if (!quality.eligible) {
      await saveCmsItem({
        ...candidate,
        type: "news-candidate",
    ...
    return {
      ...
      status: generateOnly && quality.eligible ? "preview_ready" : (quality.eligible ? run.status : "candidate_rejected"),
      published: 0,
      failures: quality.failures,
      slug: candidate.slug,
      ...diagnostics,
