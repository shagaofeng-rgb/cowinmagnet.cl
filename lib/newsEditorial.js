...
const REQUIRED_APPROVED_CANDIDATES = 6;
...
async function publishOne({ dryRun = false, force = false, generateOnly = false, context = {} } = {}) {
  ...
  const approvedCandidateCount = state.candidates.filter((item) => item.editorialApproved === true && item.status !== "rejected").length;
  if (!generateOnly && approvedCandidateCount < REQUIRED_APPROVED_CANDIDATES) {
    return {
      success: true,
      status: "awaiting_six_approved_candidates",
      published: 0,
      approved_candidate_count: approvedCandidateCount,
      required_approved_candidates: REQUIRED_APPROVED_CANDIDATES
    };
  }
