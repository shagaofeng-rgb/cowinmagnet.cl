...
import { DEFAULT_NEWS_SITE_ID, getNewsSiteConfig } from "@/lib/newsSiteConfig.mjs";
import { sourceCatalogForNewsSite } from "@/lib/newsSourceCatalog.mjs";
...
export async function runNewsSourceHealthCheck({ siteId = DEFAULT_NEWS_SITE_ID, limit = MAX_BATCH_SIZE, force = false, trigger = "cron" } = {}) {
  ...
  return withCmsAdvisoryLock(`news:source-health:${siteId}`, async () => {
    ...
    }).sort((left, right) => Number(Boolean(right.configuredPriority)) - Number(Boolean(left.configuredPriority)) || Number(Boolean(right.rssOrApiUrl)) - Number(Boolean(left.rssOrApiUrl)) || left.sourceOrdinal - right.sourceOrdinal).slice(0, safeLimit);
    const validations = [];
