...
async function runDatabaseJob(options) {
  ...
  try {
    ...
    const lockResult = await client.query("SELECT pg_try_advisory_lock($1) AS locked", [JOB_LOCK_ID]);
    ...
    if (locked) await client.query("SELECT pg_advisory_unlock($1)", [JOB_LOCK_ID]).catch(() => {});
    client.release();
  }
}
