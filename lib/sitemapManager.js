...
async function runDatabaseJob(options) {
  ...
  try {
    ...
    const submission = options.dryRun ? null : await submitIfRequested(client, options.submit && (changed || options.force));
...
async function runLocalJob(options) {
  return withFileLock(LOCAL_LOCK, async () => {
    ...
    const changed = Boolean(resolved.added.length || resolved.modified.length || resolved.deleted.length || options.force);
    const submission = options.dryRun || !options.submit || !(changed || options.force)
      ? null
  ...
  if (db) {
    ...
    try {
      ...
      if (!result.rows[0] || result.rows[0].source_version !== SOURCE_VERSION) {
        client.release();
        client = null;
        await runSitemapJob({ trigger: result.rows[0] ? "deployment-refresh" : "public-request" });
