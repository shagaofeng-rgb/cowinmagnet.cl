...
const GOOGLE_SUBMISSION_INTERVAL_MS = 72 * 60 * 60 * 1000;
...
const SOURCE_VERSION = process.env.VERCEL_GIT_COMMIT_SHA || process.env.SITE_CONTENT_UPDATED_AT || "local";
...
function getPool() {
  ...
  if (!pool) {
    ...
    pool.on("error", (error) => {
      console.warn("[sitemap] idle database connection error", error?.message || error);
    });
  }
  return pool;
}
...
async function ensureSchema(client) {
  ...
  try {
    ...
    `);
    await client.query(`ALTER TABLE ${SNAPSHOT_TABLE} ADD COLUMN IF NOT EXISTS source_version TEXT`);
    await client.query(`
...
async function collectCandidates() {
  ...
  const pagePaths = ["", "products", "industries", "solutions", "markets", "technical-support", "downloads", "about", "news", "blog", "contact", "request-a-quote"];
  const groups = { pages: [], categories: [], products: [], posts: [] };

  // `/es` remains reachable for users and existing links, but it is not a
  // separate canonical sitemap target while its primary content duplicates
  // the Chilean Spanish version.
  for (const locale of INDEX_LOCALES) {
    ...
    for (const item of cmsNews) {
      if (item.status !== "published" || !isLocaleContentIndexable(item, locale)) continue;
    ...
    for (const item of cmsBlogs) {
      if (item.status !== "published" || !isLocaleContentIndexable(item, locale)) continue;
      groups.posts.push(candidate("posts", `/${locale}/blog/${item.slug}`, { locale, item }, item.updatedAt || item.publishedAt || item.createdAt));
    }
  }
...
async function submitIfRequested(client, submit) {
  if (!submit) return null;
  const previous = await client.query(`
    SELECT finished_at
    FROM ${RUN_TABLE}
    WHERE submitted = TRUE AND status = 'success'
    ORDER BY finished_at DESC
    LIMIT 1
  `);
  const lastSubmittedAt = previous.rows[0]?.finished_at ? new Date(previous.rows[0].finished_at).getTime() : 0;
  const nextEligibleAt = lastSubmittedAt + GOOGLE_SUBMISSION_INTERVAL_MS;
  if (lastSubmittedAt && Date.now() < nextEligibleAt) {
    return {
      success: false,
      skipped: true,
      reason: "Google sitemap submission is rate-limited to once every 72 hours.",
      lastSubmittedAt: new Date(lastSubmittedAt).toISOString(),
  ...
  try {
    ...
    if (!options.dryRun) {
      ...
      try {
        ...
        await client.query(`
          INSERT INTO ${SNAPSHOT_TABLE} (file_name, xml, checksum, url_count, byte_size, source_version, generated_at)
          SELECT item.file_name, item.xml, item.checksum, item.url_count, item.byte_size, item.source_version, item.generated_at::timestamptz
          FROM jsonb_to_recordset($1::jsonb) AS item(file_name text, xml text, checksum text, url_count integer, byte_size integer, source_version text, generated_at text)
        ...
        `, [JSON.stringify(documents.map((doc) => ({ file_name: doc.fileName, xml: doc.xml, checksum: doc.checksum, url_count: doc.urlCount, byte_size: doc.byteSize, source_version: SOURCE_VERSION, generated_at: doc.generatedAt })))]);
    ...
    const submission = options.dryRun ? null : await submitIfRequested(client, options.submit && (changed || options.force || options.submitWhenUnchanged));
...
async function runLocalJob(options) {
  return withFileLock(LOCAL_LOCK, async () => {
    ...
    const changed = Boolean(resolved.added.length || resolved.modified.length || resolved.deleted.length || options.force);
    const submission = options.dryRun || !options.submit || !(changed || options.force || options.submitWhenUnchanged)
      ? null
      : await submitSitemapToSearchConsole(`${BASE_URL}/sitemap.xml`);
    const finishedAt = new Date().toISOString();
    const run = summarizeRun({ id, trigger: options.trigger, startedAt, finishedAt, resolved, documents, split, submission, dryRun: options.dryRun, changed });
...
export async function runSitemapJob({ trigger = "manual", force = false, dryRun = false, submit = false, submitWhenUnchanged = false } = {}) {
  const options = { trigger, force: Boolean(force), dryRun: Boolean(dryRun), submit: Boolean(submit), submitWhenUnchanged: Boolean(submitWhenUnchanged) };
  return getPool() ? runDatabaseJob(options) : runLocalJob(options);
}

async function getLocalSitemapDocument(fileName) {
  try {
    const xml = await fs.readFile(path.join(LOCAL_DIR, fileName), "utf8");
    const state = await readLocalState();
    const meta = (state.snapshots || []).find((item) => item.file_name === fileName) || {};
    return { file_name: fileName, xml, ...meta };
  } catch {
    const result = await runLocalJob({ trigger: "public-request-fallback" });
    if (!result.success) return null;
    const xml = await fs.readFile(path.join(LOCAL_DIR, fileName), "utf8").catch(() => "");
    return xml ? { file_name: fileName, xml } : null;
  }
}
...
export async function getSitemapDocument(fileName = "sitemap.xml") {
  ...
  if (db) {
    ...
    try {
      ...
      await ensureSchema(client);
      let result = await client.query(`SELECT file_name, xml, checksum, url_count, byte_size, source_version, generated_at FROM ${SNAPSHOT_TABLE} WHERE file_name = $1`, [safeName]);
      if (!result.rows[0] || result.rows[0].source_version !== SOURCE_VERSION) {
        client.release();
        client = null;
        const deploymentRefresh = Boolean(result.rows[0]);
        await runSitemapJob({
          trigger: deploymentRefresh ? "deployment-refresh" : "public-request",
          // A new deployment can change the indexed URL set. Submit that
          // refreshed sitemap once; the manager's 72-hour limiter protects
          // the API from ordinary crawler traffic.
          submit: deploymentRefresh
        });
        const retryClient = await db.connect();
        try {
          result = await retryClient.query(`SELECT file_name, xml, checksum, url_count, byte_size, source_version, generated_at FROM ${SNAPSHOT_TABLE} WHERE file_name = $1`, [safeName]);
        } finally {
          retryClient.release();
        }
      }
      ...
      // Crawlers should still receive a valid sitemap while a managed database is unavailable.
      console.warn("[sitemap] database read failed; using generated fallback", error?.message || error);
      return getLocalSitemapDocument(safeName);
    } finally {
  ...
  if (db) {
    ...
    try {
      ...
      const [snapshots, runs] = await Promise.all([
        client.query(`SELECT file_name, url_count, byte_size, source_version, generated_at FROM ${SNAPSHOT_TABLE} ORDER BY file_name`),
