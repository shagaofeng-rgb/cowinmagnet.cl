...
function parseCatalog(html) {
  ...
  const products = cards.map((card) => {
    ...
    return { slug, name, category, image, imageGallery: [image] };
  }).filter((product) => product.slug && product.name && product.category && product.image);

  const unique = new Map(products.map((product) => [product.slug, product]));
  if (unique.size < 1 || unique.size !== products.length) {
    throw new Error(`Catalog parse rejected: ${products.length} cards, ${unique.size} unique products`);
  }
